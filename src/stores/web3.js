...
export const mintFx = attach({
  effect: createEffect(
    async ({
      ...
      try {
        // const tokenPrice = Web3.utils.toWei("0.02", 'ether');
        const tokenPrice = Web3.utils.toWei("0.02", "ether");
        ...
        try {
          ...
          if (
            e.message &&
            e.message.includes("current network does not support EIP-1559")
          ) {
            console.log(
              "Mint failed because of EIP-1559. Sending as type 0 now."
            );
          } else {
            throw e;
          }
...
export const giftFx = attach({
  effect: createEffect(
    async ({ $ethereum, $contract, $isChainOK, $account, $web3 }) => {
      if (!$ethereum)
        throw errorMessageFx(
          "Non-Ethereum browser detected. You should consider trying MetaMask!"
        );
      ...
      try {
        ...
        // console.log(canGift,' - WL ')
        if (!canGift) return errorMessageFx('You are not CoolCats, CoolDogs or Alien frens Holder');
        const canMaxGift = await $contract.methods.checkMaxFreeMint().call({
          from: $account,
        });
        // console.log(canMaxGift, ' - More then 1');
        if (!canMaxGift) return errorMessageFx("One token per wallet for free mint");
        const gasPrice = await $web3.eth.getGasPrice();
        let gasLimit;
        try {
